...
- (float) traverseViewChainForView:(UIView *)view toViewClass:(Class)theclass offset:(float)offset
{    
    if ([view.superview isKindOfClass:theclass])
    {
        return view.superview.frame.origin.y + offset;
    } else if ([view.superview isKindOfClass:[UIView class]]) {
        return [self traverseViewChainForView:view.superview toViewClass:theclass offset:view.superview.frame.origin.y + offset];
    } else {
        return offset;
    }
}

- (float) findViewOffsetToSuperviewClass:(Class)theclass
{
    return [self traverseViewChainForView:self toViewClass:theclass offset:0];
}
